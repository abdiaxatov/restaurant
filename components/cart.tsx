...
export function CartPage() {
  ...
  // Check if there are available tables and if delivery is available
  useEffect(() => {
    setIsLoading(true)
    ...
    try {
      // Check for available tables - use a simpler query to avoid index requirements
      tablesUnsubscribe = onSnapshot(
        query(collection(db, "tables"), where("status", "==", "available")),
        (snapshot) => {
          setHasAvailableTables(!snapshot.empty)

          // If no tables are available, default to delivery
          if (snapshot.empty) {
  ...
  const handlePlaceOrder = async () => {
    ...
    if (orderType === "table" && !hasAvailableTables) {
      toast({
        title: "Stollar mavjud emas",
        description: "Hozirda bo'sh stollar mavjud emas. Iltimos, yetkazib berish xizmatidan foydalaning.",
        variant: "destructive",
      })
      return
    }
              ...
              <Card>
                ...
                <CardContent>
                  {isLoading ? (
                    ...
                    <Tabs
                      defaultValue={hasAvailableTables ? "table" : "delivery"}
                      value={orderType}
                      onValueChange={(value) => setOrderType(value as "table" | "delivery")}
                    >
                      <TabsList className="mb-4 grid w-full grid-cols-2">
                        <TabsTrigger value="table" disabled={!hasAvailableTables && !isDeliveryAvailable}>
                          Stol buyurtmasi
                        </TabsTrigger>
                        <TabsTrigger value="delivery" disabled={!isDeliveryAvailable && !hasAvailableTables}>
                          Yetkazib berish
                        </TabsTrigger>
                      </TabsList>
                      ...
                      <TabsContent value="table" className="space-y-4">
                        {!hasAvailableTables ? (
                          <Alert variant="destructive" className="mb-4">
                            ...
                            <AlertTitle>Stollar mavjud emas</AlertTitle>
                            <AlertDescription>
                              Hozirda bo'sh stollar mavjud emas. Iltimos, yetkazib berish xizmatidan foydalaning.
                            </AlertDescription>
                          </Alert>
                        ) : (
                          <>
                ...
                <CardFooter className="flex flex-col">
                  ...
                  <Button
                    ...
                    disabled={
                      isSubmitting ||
                      items.length === 0 ||
                      (orderType === "table" && !hasAvailableTables) ||
